        ...
        children: [
          ...
          TextField(
            ...
            decoration: const InputDecoration(
              labelText: 'Título',
              hintText:
                  'Ej: Entrené Muay Thai / Caí en paja / Deep work 90 min',
          ...
          DropdownButtonFormField<LifeArea?>(
            value:
                (_type == ActionType.goodHabit || _type == ActionType.badHabit)
                ? _area
                : null,
            items: [
          ...
          FilledButton.icon(
            onPressed: () async {
              ...
              await controller.gameController.addAction(entry);
          ...
          children: [
            ...
            ...presets.map(
              (p) => ListTile(
                title: Text(p.title),
                subtitle: Text(
                  'HP ${p.hp}, Varos ${p.varos}, XP ${p.xp} • ${p.area.label}',
                ),
                onTap: () {
                  setState(() {
                    _title.text = p.title;
...
class _TypePicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SegmentedButton<ActionType>(
      segments: const [
        ButtonSegment(
          value: ActionType.goodHabit,
          label: Text('Bueno'),
          icon: Icon(Icons.trending_up),
        ),
        ButtonSegment(
          value: ActionType.badHabit,
          label: Text('Malo'),
